...
const struct attm_desc f030_att_db[F030S_IDX_NB] =
{
    ...
    [F030S_IDX_F031_VAL_VAL]   =   {ATT_USER_SERVER_CHAR_F031,PERM(RD, ENABLE), PERM(RI, ENABLE), F030_CHAR_DATA_LEN *sizeof(uint8_t)},

    [F030S_IDX_F031_USER_DESC] =   {ATT_DESC_CHAR_USER_DESCRIPTION,PERM(RD, ENABLE), PERM(RI, ENABLE), F030_CHAR_DATA_LEN *sizeof(uint8_t)},

    
	// f032 value Characteristic Declaration
	[F030S_IDX_F032_VAL_CHAR]  =   {ATT_DECL_CHARACTERISTIC, PERM(RD, ENABLE), 0, 0},
    // f031 Level Characteristic Value
    [F030S_IDX_F032_VAL_VAL]   =   {ATT_USER_SERVER_CHAR_F032, PERM(WRITE_COMMAND, ENABLE), PERM(RI, ENABLE), F030_CHAR_DATA_LEN * sizeof(uint8_t)},

    	// f032 value Characteristic Declaration
	[F030S_IDX_F033_VAL_CHAR]  =   {ATT_DECL_CHARACTERISTIC, PERM(RD, ENABLE), 0, 0},
    // f031 Level Characteristic Value
    [F030S_IDX_F033_VAL_VAL]   =   {ATT_USER_SERVER_CHAR_F033, PERM(WRITE_REQ, ENABLE), PERM(RI, ENABLE), F030_CHAR_DATA_LEN * sizeof(uint8_t)},
	
	// f031 Level Characteristic Declaration
	[F030S_IDX_F034_VAL_CHAR]  =   {ATT_DECL_CHARACTERISTIC, PERM(RD, ENABLE), 0, 0},
    // f031 Level Characteristic Value
    [F030S_IDX_F034_VAL_VAL]   =   {ATT_USER_SERVER_CHAR_F034, PERM(WRITE_COMMAND, ENABLE), PERM(RI, ENABLE), F030_CHAR_DATA_LEN * sizeof(uint8_t)},
	...
	// f031 Level Characteristic - Client Characteristic Configuration Descriptor
	[F030S_IDX_F035_VAL_IND_CFG] = {ATT_DESC_CLIENT_CHAR_CFG,  PERM(RD, ENABLE)|PERM(WRITE_REQ, ENABLE), 0, 0},
...
static uint8_t f030s_init (struct prf_task_env* env, uint16_t* start_hdl, uint16_t app_task, uint8_t sec_lvl,  struct f030s_db_cfg* params)
{
    ...
    {
        //Set optional permissions
       // if((params->features & 0x01) == F030_F034_VAL_NTF_SUP)
        {
		...
		//if((params->features & 0x02) == F030_F033_LVL_NTF_SUP)
		{
            ...
            attm_att_set_permission(shdl + F030S_IDX_F035_VAL_VAL, perm, 0);
		}
    }
...
void f030s_notify_f034_val(uint8_t conidx,struct f030s_env_tag* f030s_env, struct f030s_f0345_val_upd_req const *param)
{
...
void f030s_indicate_f035_val(uint8_t conidx,struct f030s_env_tag* f030s_env, struct f030s_f0345_val_upd_req const *param)
{
    // Allocate the GATT notification message
    struct gattc_send_evt_cmd *val = KE_MSG_ALLOC_DYN(GATTC_SEND_EVT_CMD,
            KE_BUILD_ID(TASK_GATTC, conidx), prf_src_task_get(&(f030s_env->prf_env),0),
    ...
    val->handle = f030s_get_att_handle(F030S_IDX_F035_VAL_VAL);
    // pack measured value in database
    val->length = param->length;
	memcpy(&val->value[0],&param->value[0],param->length);
    // send notification to peer device
...
uint16_t f030s_get_att_handle( uint8_t att_idx)
{
    ...
    if(att_idx <= F030S_IDX_F035_VAL_IND_CFG)
    {
        handle += att_idx;
    }
	else
...
uint8_t f030s_get_att_idx(uint16_t handle, uint8_t *att_idx)
{
    ...
    if(handle <= (hdl_cursor1 + F030S_IDX_F035_VAL_IND_CFG))
    {
