...
void app_f040_add_f040s(void)
{
   ...
   struct gapm_profile_task_add_cmd *req = KE_MSG_ALLOC_DYN(GAPM_PROFILE_TASK_ADD_CMD,
    ...
    db_cfg->f041_desc_len = strlen("f041_only_read");
   
    memcpy(db_cfg->f041_desc,"f041_only_read",strlen("f041_only_read"));
...
{
    // Allocate the message
    struct f040s_f0445_val_upd_req * req = KE_MSG_ALLOC(F040S_F045_VALUE_UPD_REQ,
                                                        prf_get_task_from_id(TASK_ID_F040S),
                                                        KE_BUILD_ID(TASK_APP, conidx),
                                                        f040s_f0445_val_upd_req);
    // Fill in the parameter structure	
                                               ...
                                               ke_task_id_t const src_id)
{
	...
	{
		//ke_timer_clear(F040S_F041_LEVEL_PERIOD_NTF,dest_id);
	}else
	{
		//ke_timer_set(F040S_F041_LEVEL_PERIOD_NTF,dest_id , 100);
	}
    
    return (KE_MSG_CONSUMED);
}

static int f040s_f045_val_ind_cfg_ind_handler(ke_msg_id_t const msgid,
                                               struct f040s_f045_val_ind_cfg_ind const *param,
                                               ke_task_id_t const dest_id,
                                               ke_task_id_t const src_id)
{
	uart_printf("f045->param->ind_cfg = %x\r\n",param->ind_cfg);
...
static int f045_val_upd_rsp_handler(ke_msg_id_t const msgid,
                                      struct f040s_f0445_val_upd_rsp const *param,
                                      ke_task_id_t const dest_id,
                                      ke_task_id_t const src_id)
{
                                     ...
                                     ke_task_id_t const src_id)
{
    ...
    {
        app_f045_send_ind(param->conidx,20,buf);
    }
#if 0    
...
const struct ke_msg_handler app_f040_msg_handler_list[] =
{
    ...
    {F040S_F045_VALUE_IND_CFG_IND,  (ke_msg_func_t)f040s_f045_val_ind_cfg_ind_handler},
    {F040S_F044_VALUE_UPD_RSP,      (ke_msg_func_t)f044_val_upd_rsp_handler},
    {F040S_F045_VALUE_UPD_RSP,      (ke_msg_func_t)f045_val_upd_rsp_handler},
